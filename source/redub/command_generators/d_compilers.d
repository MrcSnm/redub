...
BuildConfiguration pkgconfigFromLibs(string[] libs, string[] modifiedFlagsFromPkgConfig)
{
    ...
    Tuple!(Pid, string)[] checkLibs;
    Tuple!(Pid, string)[] recheckLibs;

    string[] baseLibs;
    string[] pkgConfigLibs;

    foreach(l; libs)
        checkLibs~= tuple(spawnProcess(pkgconfig_bin~" --exists "~l), l);

    ///Tries to find libraries with pkgconfig
    foreach(check; checkLibs)
    {
        if(check[0].wait != 0)
            recheckLibs~= tuple(spawnProcess(pkgconfig_bin~" --exists lib"~check[1]), "lib"~check[1]);
        else
            pkgConfigLibs~= check[1];
    }
    ///Tries to find libraries with pkgconfig after prefixing with "lib"
    foreach(rec; recheckLibs)
    {
        if(rec[0].wait != 0)
            baseLibs~= rec[1];
        else
    ...
    ///At this point, all the processes should have finished.
    // //Executes pkgconfig again for each library found to get the dependencies.

    BuildConfiguration ret;

    if (pkgConfigLibs.length)
    {
        import redub.logging;
        vlog("Using pkg-config to resolve library flags for ", pkgConfigLibs);
        auto libflags = execute([pkgconfig_bin, "--libs"] ~ pkgConfigLibs);
        enforce(libflags.status == 0, "pkg-config exited with error code "~ libflags.status.to!string ~ ": "~ libflags.output);
        foreach (string f; libflags.output.splitter())
        {
            if (f.startsWith("-L-L"))
                ret.linkFlags ~= f[2 .. $];
